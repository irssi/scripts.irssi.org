...
Irssi::settings_add_str('gmail', 'gmail_layer', 'tls');
...
/set gmail_layer <tls|ssl> (default is tls)

This script can be used together with trigger.pl as in the following example:

/TRIGGER ADD -topics -command 'GMAIL -to pab_24n\@hotmail.com -subject "New topic in \$C\@\$T" -html -body "<b>\$N</b> changed topic of <u>\$C</u> to:<br><i>\$M</i>"'
SCRIPTHELP_EOF
			,MSGLEVEL_CLIENTCRAP);
		Irssi::signal_stop;
	}
...
sub cmd_gmail {
	...
	my $layer    = lc(Irssi::settings_get_str("gmail_layer"));
